...
static bool do_write(const char *image_path, const char *serial_prefix, const char *port_path, bool dryrun, bool force_erase);
...
int subcommand_write(int argc, char * const argv[])
{
    ...
    return do_write(argv[optind], serial_prefix, port_path, dryrun, force_erase) ? EXIT_SUCCESS : EXIT_FAILURE;
}
...
static bool do_write(const char *image_path, const char *serial_prefix, const char *port_path, bool dryrun, bool force_erase)
{
    ...
    if (!modified) {
        log_verbose("No changes were made, flash contents already match the provided image");
    } else if (dryrun) {
